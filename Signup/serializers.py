...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
        fields = ('first_name', 'last_name','id', 'email','user_type','email_otp','email_otp_expiry_time','password')
    def create(self, validated_data):
        user = User.objects.create_user(**validated_data)
        user.email_otp = generate_random_otp(6)
        user.email_otp_expiry_time=timezone.now()+timedelta(seconds=60)
        user.save()
        return user
...
class ChangeUserPasswordSerializer(serializers.ModelSerializer):
    ...
    def validate(self, initial_data):
        ...
        if initial_data['password'] != initial_data['confirm_password']:
            raise Exception("Your New Password and Confirm Password do not match.")
        return initial_data
